...
var websocket_server: WebSocketServer = null

func _ready() -> void:
    setup_api_servers()

func setup_api_servers() -> void:
    pass

func register_agent(agent_id: String, agent_type: AgentState.AgentType = AgentState.AgentType.RULE_BASED, agent_name: String = "") -> bool:
    ...
    var new_agent: AgentState = AgentState.new()
...
func update_agent_position(agent_id: String, new_position: Vector3) -> void:
    var agent: AgentState = get_agent(agent_id)
    if agent:
        agent.position = new_position

func update_agent_status(agent_id: String, new_status: AgentState.AgentStatus) -> void:
    var agent: AgentState = get_agent(agent_id)
    if agent:
...
func get_agent_perception(agent_id: String) -> Dictionary:
    ...
    for other_id in active_agents:
        ...
        var other_agent: AgentState = agents[other_id]
        ...
        if distance <= agent.perception_range:
            perception["nearby_agents"].append({
                "agent_id": other_id,
                "distance": distance,
                "position": [other_agent.position.x, other_agent.position.y, other_agent.position.z],
                "status": AgentState.AgentStatus.keys()[other_agent.status]
            })
...
func broadcast_event(event: Dictionary) -> void:
    ...
    var pos_data: Array = event.get("position", [0, 0, 0])
    event_position = Vector3(pos_data[0], pos_data[1], pos_data[2])
    
    for agent_id in active_agents:
        var agent: AgentState = agents[agent_id]
...
func update_all_agents(delta: float) -> void:
    for agent_id in active_agents:
        var agent: AgentState = agents[agent_id]
        if agent.status == AgentState.AgentStatus.ACTING:
