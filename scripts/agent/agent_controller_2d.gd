...
var agent_state: AgentState2D = null
...
func _ready() -> void:
    ...
    AgentManager2D.agent_action_requested.connect(_on_action_requested)

func initialize_agent_state() -> void:
    agent_state = AgentState2D.new()
    agent_state.agent_id = agent_id
    agent_state.agent_type = AgentState2D.AgentType.RULE_BASED
    agent_state.position = global_position
    
    AgentManager2D.register_agent(agent_id, agent_state.agent_type, agent_state.agent_name)
...
func execute_move_action(action: Dictionary, delta: float) -> void:
    var target_position_data: Array = action.get("target_position", [0, 0])
    var target_position: Vector2 = Vector2(target_position_data[0], target_position_data[1])
    ...
    if distance < 5.0:
        ...
        current_action = {}
        agent_state.velocity = Vector2.ZERO
        return
        ...
        if nearby.size() > 0:
            ...
            if agent_state.relationships.get(target_id, 0.0) > 0:
                agent_state.queue_action({
                    ...
                    "type": "move",
                    "target_position": nearest.get("position", [0, 0])
                })
...
func _on_action_requested(requested_agent_id: String, action: Dictionary) -> void:
    if requested_agent_id == agent_id:
        ...
        agent_state.status = AgentState2D.AgentStatus.ACTING
...
func stop_current_action() -> void:
    ...
    agent_state.status = AgentState2D.AgentStatus.IDLE
    agent_state.velocity = Vector2.ZERO